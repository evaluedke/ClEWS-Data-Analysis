...
modNO <- glm(`SPV [MW/MW]`~ `GHI [Wm-2]` , data=Norway)
...
modES <- glm(`SPV [MW/MW]`~ `GHI [Wm-2]` , data=Spain)
...
spainplot = ggplot(Spain,aes(x = `GHI [Wm-2]`, 
  ...
  annotate("text", x = 191, y = 7010000 , label = label3, col = "orange",size = 8/.pt)+
  labs(y = "Solar photovoltaic power generation [MW/MW]", x = "Surface downwelling shortwave radiation [Wm-2]") + 
  #ylim(500000,50000000)+
  theme_bw()+
  theme(axis.text.y = element_text(angle=90, vjust=.5, hjust=1),legend.position = "top")
...
```
-Germany has highest solar photovoltaic power generation 
-spain higher surface downwelling shortwave radiation 
-linear dependance between GHI and SPV
-however, even though spain has more GHI, less SPV -> solar photovoltaic power not as present as in germany (sources?), had potential to generate a lot more solar power
-Norway has lower radiation but much lower solar photovoltaic power generation --> higher potential 


```{r}
subplot <- newenergydata %>% filter(Country=="DE" |
                                    Country=="NO" |
                                    Country=="ES")


ggplot(subplot,aes(x = `GHI [Wm-2]`, 
                 y= `SPV [MW/MW]`,
                 col=Country)) +
...
-does SPV only depend on radiation? Multivariate regression? which one? 
-AIC --> which one is better
-plot both in 1 diagram or even 3D 
