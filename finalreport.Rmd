...
```{r echo=FALSE,message=FALSE, warning=FALSE}
knitr::opts_chunk$set(root.dir = "~/Uni Potsdam/ClEWS/1. Semester/Data Analysis and Management in Earth System Science")
```

```{r echo=FALSE,message=FALSE, warning=FALSE}
setwd("~/Uni Potsdam/ClEWS/1. Semester/Data Analysis and Management in Earth System Science/Data/")
```
...
Summaries of the wind power generation and wind speed at 100m deliver an overwiew of the parameter for each country. Denmark shows the highest mean in wind speed, while Switzerland has the lowest mean wind speed at 100m. Therefore, it is interesting to compare these countries to Germany.

```{r}
summary(windpoweronshore_his)
```

```{r}
summary(wind_speed_100_his)
```

```{r}
#select only Germany from each dataframe
...
`EDM [MWh]` <- electricity_demand_his %>% select(1,15) 
colnames(`EDM [MWh]`)[2] <- "EDM [MWh]"

`GHI [Wm-2]` <- irradiance_his %>% select(1,16)
colnames(`GHI [Wm-2]`)[2] <- "GHI [Wm-2]"

`HRE [W]` <- hydropower_reservoir_his %>% select(1,16)
colnames(`HRE [W]`)[2] <- "HRE [W]"

`HRO [W]` <- hydropower_ro_his %>% select(1,16)
colnames(`HRO [W]`)[2] <- "HRO [W]"

`MSL [Pa]` <- sealevelpressure_his %>% select(1,16)
colnames(`MSL [Pa]`)[2] <- "MSL [Pa]"

`SPV [MW/MW]` <- solarpvpwr %>% select(1,16)
colnames(`SPV [MW/MW]`)[2] <- "SPV [MW/MW]"

`TA [K]` <- air_temperature_his %>% select(1,16)
colnames(`TA [K]`)[2] <- "TA [K]"

`TP [m]` <- total_pr_his%>% select(1,16)
colnames(`TP [m]`)[2] <- "TP [m]"

`WON [MW/MW]` <- windpoweronshore_his %>% select(1,16)
colnames(`WON [MW/MW]`)[2] <- "WON [MW/MW]"

`WS-10m [ms-1]` <- wind_speed_10_his %>% select(1,16)
colnames(`WS-10m [ms-1]`)[2] <- "WS-10m [ms-1]"

`WS-100m [ms-1]` <- wind_speed_100_his %>% select(1,16)
colnames(`WS-100m [ms-1]`)[2] <- "WS-100m [ms-1]"
